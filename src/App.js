    ...
    <div
      ...
      >
        ...
        <div className="flex justify-center items-center">
          <div className={`w-full max-w-3xl min-h-[600px] backdrop-blur-sm rounded-2xl shadow-xl p-6 ${darkTheme ? "bg-slate-800/90 border border-slate-700" : "bg-white/90 border border-slate-200"}`}>
            <div className={`flex justify-between items-end mb-5 pb-3 ${darkTheme ? "border-b border-slate-700" : "border-b border-slate-200"}`}>
              <h1 className={`text-4xl md:text-5xl font-bold tracking-tight drop-shadow-sm ${darkTheme ? "text-amber-400" : "text-amber-600"}`}>

                Action Plan
              </h1>
              <p className={`text-lg ${darkTheme ? "text-slate-300" : "text-slate-600"}`}>
                <span className={`font-bold text-2xl ${darkTheme ? "text-slate-100" : "text-slate-800"}`}>
            ...
            <div className={`rounded-xl p-4 min-h-[300px] space-y-2.5 ${darkTheme ? "border border-slate-700 bg-slate-900/60" : "border border-slate-200 bg-slate-50/50"}`}>
              {actions.length > 0 ? (
                actions.map((act) => (
                  <Task
                    action={act}
                ...
                <p className={`flex justify-center items-center h-full text-sm italic ${darkTheme ? "text-slate-500" : "text-slate-400"}`}>
                  Please add actions
                </p>
              )}
            </div>
